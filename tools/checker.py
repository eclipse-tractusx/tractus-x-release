...
# def check_old(row):
#     print(f"Checking {parsed_row['type']}: {parsed_row['type_name']}")
#
#     component_link = row['Component'].strip().split(']')[1][1:-1]
#     if not check_link(component_link):
#         print(f"  Error: Component link is not working for component {component_name}.")
#
#     # TODO: here could be multiple version_links
#     # sample Frontend: [v1.3.1](https://link-to-version)<br/> Backend: [v1.3.1](https://link-to-version)
#     app_kit_line = row['App-/KIT Version (s)'].strip()
#     print(f"app_kit_line: {app_kit_line}")
#     if "<br/>" in app_kit_line:
#         print("Multiline app_kit_line")
#         app_kit_links = [version_link.strip().split(']')[1][1:-1] for version_link in app_kit_line.split("<br/>")]
#         # app_kit_links = [version_link.strip().split(']')[1][1:-1] in app_kit_line.split("<br/>")]
#     else:
#         app_kit_links = [row['App-/KIT Version (s)'].strip().split(']')[1][1:-1]]
#     # check if multiline by regex
#     # per line get version_link
#     # version_link = row['App-/KIT Version (s)'].strip().split(']')[1][1:-1]
#
#     for version_link in app_kit_links:
#         # Check if version link is working
#         if not check_link(version_link):
#             print(f"  Error: APP/KIT Version link is not working for component {component_name}.")
#
#     # Extract chart name and version if available
#     if "n/a" not in helm_chart:
#         print(f"helm chart {helm_chart}")
#         chart_name = helm_chart.split(':')[0].strip()
#         chart_version = helm_chart[helm_chart.index("[") + 1:helm_chart.index("]")]
#         print(f"chart_version: {chart_version}")
#         chart_version_link = helm_chart.split('[')[-1].split(']')[0].strip()
#
#         # Check if Helm chart and app version are latest
#         latest_chart_version, latest_app_version = check_versions(chart_name)
#         if latest_chart_version and latest_app_version:
#             print(f"  Latest Helm Chart Version: {latest_chart_version}")
#             print(f"  Latest App Version: {latest_app_version}")
#         else:
#             print("  Error: Could not retrieve latest Helm chart or app version.")
